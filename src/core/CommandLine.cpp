...
{
    if (value.empty()) throw CommandLineError("--frames requires a positive integer");
    std::string narrow;
    narrow.reserve(value.size());
    for (const wchar_t character : value)
    {
        if (character < L'0' || character > L'9') throw CommandLineError("--frames requires a positive integer");
...
CommandLineOptions parse_command_line(std::span<const std::wstring_view> arguments)
{
    ...
    for (std::size_t index = 0; index < arguments.size(); ++index)
    {
        ...
        else if (argument == L"--frames")
        {
            ...
            options.frame_limit = parse_positive_integer(require_value(arguments, index, "--frames"));
        }
        ...
        else if (argument == L"--diagnostic")
        {
            ...
            else throw CommandLineError("--diagnostic must be shaded, normals, uv, or bounds");
        }
        else
        {
...
std::string usage_text()
{
    return
        ...
        "  --diagnostic <mode>            shaded, normals, uv, or bounds.\n"
